...
- 后端：微信云开发 CloudBase，云数据库集合 `progress`（打卡记录）、`schedule`（调课覆盖，见 D015）。
- **小程序端数据库查询单次最多返回 20 条**（官方硬限制，传更大的 `limit` 会被**静默截断**，不报错）。任何 `skip`/`limit` 分页的**步长必须等于 20**，否则会整段漏读记录。曾因此在旧数据回补里漏掉第 20 条之后的所有记录（2026-09-23 修复）。
...
- 本地缓存（wx storage）：**只剩回补标记一项**。
  - `legacyPagesFixed_v2`：旧数据回补一次性标记（布尔）。键带版本号，是为了让分页 bug 修好后回补能重跑一次。
  - 调课覆盖**已不在本地**：2026-09-23 移到云集合 `schedule`，两人共享且当周有效（见 D015，取代 D003）。
...
- ~~已有 `customSchedule` 缓存结构兼容~~ —— **该条已失效**：调课覆盖 2026-09-23 迁到云集合 `schedule`（D015），
  老设备本地缓存里的 `customSchedule` **不再被读取**（未主动删除，留着无害）。此处保留删除线是为了让
  未来 Agent 知道"这条约束曾经存在、为什么没了"，不要试图去兼容它。
