...
- 课表页：极简时间轴 + 起止时间 + 点卡片底部 actionSheet 调课（当时存本地 `customSchedule`；**2026-09-23 已改为云集合 `schedule`，见 D015**）。
...
## 维护期增量（不属任何 Phase，用户直接提出的需求）

> Phase 3 虽已暂缓，但用户在使用中直接提出了一批需求。这些**不是** Phase 3 的候选方向，
> 单独列在这里，避免与 Phase 3 的"未开工"状态混淆。

| ID | 内容 | 状态 | 验证情况 |
|---|---|---|---|
| M1 | 打卡页提交确认弹窗 + 撤销按钮（D012） | ✅ Completed | 状态机测试 45 项 + 用户验收（2026-09-23） |
| M2 | 打卡页横向进度轴，推荐课件居中（D013） | ✅ Completed | 居中算法复算 + 换行容量复算 + 用户验收 |
| M3 | 打卡页一屏免滚动布局（D014） | ✅ Completed | 垂直预算 5 机型复算 + 用户验收 |
| M4 | 课表页左右滑动切周（D014） | ✅ Completed | 手势测试 16 项 + 用户验收 |
| M5 | 调课覆盖迁移到云库 + 当周自动失效（D015，**取代 D003**） | ✅ Completed | 云端存储测试 19 项 + 用户验收 |
| M6 | 修复旧数据回补的分页 bug（D016） | ✅ Completed | 造 23 条脏数据验证全部回补 |
...
| ID | 事项 | 状态 |
|---|---|---|
| O1 | `progress` **与 `schedule`** 两个集合的写权限收敛为 openid 白名单（自定义安全规则，见 D009） | 已决定，待执行 |
| O2 | 上传代码 + 设体验版，供女友使用 | 待执行 |
| O3 | 若公开发布：填服务类目 + 提审（个人主体可选范围未验证） | 未启动 |

> O4（曾为"新建云集合 `schedule`"）：**已完成**（2026-09-23，用户已在控制台建好）。
> 权限设置同 `progress`：所有用户可读，仅创建者可读写。**注意**：预设权限下「创建」永远允许，
> 所以任何微信用户都能新增调课文档 —— O1 的白名单规则必须同时覆盖这个集合。
